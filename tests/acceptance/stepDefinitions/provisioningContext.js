...
function createUser(userId, password, displayName = false, email = false) {
  ...
  return fetch(join(backendHelper.getCurrentBackendUrl(), '/ocs/v2.php/cloud/users?format=json'), {
    method: 'POST',
    body: body,
    headers: headers
  })
...
function deleteUser(userId) {
  ...
  return fetch(join(backendHelper.getCurrentBackendUrl(), '/ocs/v2.php/cloud/users/', userId), {
    method: 'DELETE',
    headers: headers
  })
}
